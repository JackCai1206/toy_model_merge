...
class SimpleTaskDataset(Dataset):
    """Map-style dataset built on top of the full-string generator."""
    ...
    def __init__(
        ...
        seed: int,
        config: SimpleDatasetConfig | None = None,
        task_schedule: Sequence[str] | None = None,
    ) -> None:
        self.tasks = [task.upper() for task in tasks]
        self.tokenizer = tokenizer
        self.seed = seed
        self.config = config or SimpleDatasetConfig(generator=GeneratorConfig())
        self.task_schedule = (
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        for _ in range(50):
            ...
            sample = generate_sample(rng, self.config.generator)
            target_tokens = apply_task(sample.tokens, task, self.config.generator)
    ...
    def _format_example(self, sample: SimpleSample, target_tokens: Sequence[str], task: str):
