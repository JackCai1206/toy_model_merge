...
from utils.sample_complexity import (
    CallbackOnlyTrainer,
    S99Callback,
...
def compute_eval_delay(eval_steps: int, eval_jitter_fraction: float, seed: int, salt: int = 0) -> int:
    """Deterministically jitter the first eval offset based on seed and an optional salt."""

    jitter_max = int(max(0.0, min(1.0, float(eval_jitter_fraction))) * max(0, int(eval_steps)))
    if jitter_max <= 0:
        return 0
    rng = random.Random(seed * 7919 + salt)
    return rng.randint(0, jitter_max)
    ...
    with torch.no_grad():
        for start in range(0, dataset_len, batch_size):
            ...
            for row, item in enumerate(batch_items):
                ...
                per_task_hits.setdefault(item["task"], []).append(is_exact)
...
def run_iterative_training_loop(
    *,
    ...
    """Delegate to the shared recursive rollback runner."""
